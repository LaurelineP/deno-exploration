...
import { abort, debug, DrakeError, glob, log, readFile, vers, writeFile, } from "./utils.ts";
/** Drake task. */
export class Task {
    /**
     * Create a new task.
...
/** Task registry map. */
export class TaskRegistry extends Map {
    constructor() {
        super();
        this.lastDesc = "";
    }
    ...
    list() {
        ...
        for (const k of keys.sort()) {
            ...
            if (env("--list-all") && task.prereqs.length > 0) {
                msg += `\n${task.prereqs.map((prereq) => `${" ".repeat(maxLen)} ${colors.yellow(prereq)}`).join("\n")}`;
            }
            result.push(msg);
        }
        return result;
...
function logFinish(message, startTime) {
    ...
    log(`${colors.green(colors.bold(`${message}:`))} finished (${endTime -
        startTime}ms)`);
}
/* Helper functions */
/**
 ...
 *
 */
export function isNormalTask(name) {
 ...
 *
 */
export function isFileTask(name) {
 ...
 */
export function normalizePrereqs(prereqs) {
    const result = [];
...
//# sourceMappingURL=file:///Users/laurelineparis/Desktop/deno-exploration/deno_dir/gen/https/deno.land/x/drake@v1.2.3/lib/tasks.ts.js.map